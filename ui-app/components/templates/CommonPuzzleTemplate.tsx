...
import { PuzzleFaqSection } from '@/components/sections';
...
export default async function CommonPuzzleTemplate({ params }: PuzzleRouteProps) {
  ...
  const breadcrumbSchema = { '@context': 'https://schema.org', '@type': 'BreadcrumbList', itemListElement: [
    ...
    { '@type': 'ListItem', position: 2, name: category.name, item: `${SITE_URL}/${locale}/${categorySlug}/` },
    { '@type': 'ListItem', position: 3, name: puzzle.name, item: `${SITE_URL}/${locale}/${categorySlug}/${puzzle.slug}/` }
  ] };
  const puzzleJsonLd = puzzle.jsonLd ? {
    '@context': 'https://schema.org',
    '@type': puzzle.jsonLd.type,
    name: puzzle.jsonLd.name ?? puzzle.name,
    description: puzzle.jsonLd.description ?? puzzle.description,
    image: puzzle.jsonLd.image ? `${SITE_URL}${puzzle.jsonLd.image}` : `${SITE_URL}${puzzle.image}`,
    educationalUse: puzzle.jsonLd.educational_use,
    typicalAgeRange: puzzle.jsonLd.age_range
  } : null;

  return <>
    <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(breadcrumbSchema) }} />
    {puzzleJsonLd && <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(puzzleJsonLd) }} />}
    <main>
      ...
      <div className="puzzle-page-layout section" style={{ paddingTop: 0 }}>
        ...
        <div className="puzzle-details-col"><p className="eyebrow">{tp('freeCategoryPrintable', { category: category.name })}</p><h1 style={{ fontSize: 'clamp(1.8rem, 4vw, 2.8rem)' }}>{locale === 'en' && puzzle.seoH1 ? puzzle.seoH1 : tp('puzzleTitleFallback', { name: puzzle.name })}</h1><p style={{ color: 'var(--muted)', fontSize: '1.05rem', lineHeight: 1.65, margin: '14px 0 20px' }}>{puzzle.description}</p>
          ...
          <DownloadButton puzzleId={puzzle.slug} pdfUrl={puzzle.pdf} locale={locale} ariaLabel={tc('downloadAria', { name: puzzle.name })} />
