...
export async function CategoryGrid({ excludeIds, includeBlog }: { excludeIds?: string[]; includeBlog?: boolean } = {}) {
  ...
  const sortedCategories = getActiveCategories(locale)
    .filter((category) => category.id !== 'blog' && !excludeIds?.includes(category.id));
...
export function FaqBlock({ faqs, eyebrow, heading, sectionId }: { faqs: FaqItem[]; eyebrow: string; heading: string; sectionId?: string }) {
  const faqSchema = {
...
export async function Footer() {
  ...
  return (
    <footer className="footer">
      ...
      <nav aria-label={t('hubNavLabel')} className="footer-hub-links">
        <Link href={`/${locale}/ages/4-6/`}>{t('hubAges46')}</Link>
        <Link href={`/${locale}/ages/7-9/`}>{t('hubAges79')}</Link>
        <Link href={`/${locale}/ages/9-12/`}>{t('hubAges912')}</Link>
        <Link href={`/${locale}/difficulty/medium-21-60-dots/`}>{t('hubDifficultyMedium')}</Link>
        <Link href={`/${locale}/difficulty/hard-61-plus-dots/`}>{t('hubDifficultyHard')}</Link>
      </nav>
