...
import { categories, getCategory, getCrossTierPuzzles } from '@/lib/category-registry';
...
function ColoringGuide({ schemes, heading }: { schemes: any[]; heading: string }) {
  ...
  return <section id="coloring-guide" className="section" style={{ paddingTop: 0, maxWidth: 800, margin: '0 auto', padding: '0 24px 48px' }}>
    ...
    {schemes.map((scheme) => <article key={scheme.name} style={{ marginBottom: 28, paddingInlineStart: 16, borderInlineStart: '3px solid var(--pink-light, #f6a5c0)' }}>
      ...
      {Array.isArray(scheme.mapping) && <ul style={{ lineHeight: 1.8, paddingInlineStart: 20, margin: 0 }}>{scheme.mapping.map((mapping: any) => <li key={`${scheme.name}-${mapping.range}-${mapping.part}`}>
        <strong>{mapping.range}{mapping.part ? ` (${mapping.part})` : ''}:</strong>{' '}
        {mapping.hex && <span aria-hidden="true" style={{ display: 'inline-block', width: 34, height: 14, borderRadius: 7, background: mapping.hex, border: '1px solid rgba(0,0,0,0.18)', verticalAlign: 'middle', marginInlineEnd: 6 }} />}
...
export default async function CommonPuzzleTemplate({ params }: PuzzleRouteProps) {
  ...
  const pageNamespace = collectionPageNamespaces[categorySlug];
  const tPage = pageNamespace ? await getTranslations(pageNamespace) : null;
  const categoryName = tPage?.has('category') ? tPage('category') : category.name;
  ...
  const crossTier = getCrossTierPuzzles(locale, categorySlug, puzzle, 3);
  const crossTierHeading = `${localizeAge(tc, puzzle.age)} — ${difficultyLabels[puzzle.difficulty - 1]}`;
  ...
  return <>
    ...
    <main>
      ...
      {related.length > 0 && <section className="section" style={{ paddingTop: 8 }}><div className="section-heading"><Link href={`/${locale}/${categorySlug}/`} className="eyebrow" style={{ textDecoration: 'none' }}>{tPage?.has('relatedEyebrow') ? tPage('relatedEyebrow') : categoryName}</Link><h2 style={{ fontSize: 'clamp(1.5rem, 3vw, 2.2rem)' }}>{tp('youMightLike')}</h2></div><div className="puzzle-grid">{related.map((item) => <article className="puzzle-card" key={item.slug}><Link href={`/${locale}/${categorySlug}/${item.slug}/`} className="puzzle-image"><figure style={{ margin: 0 }}><ResponsiveImage src={item.image} alt={puzzleImageAlt(locale, item, 'card')} width={420} height={320} sizes="(max-width: 760px) 92vw, 30vw" /><figcaption className="sr-only">{puzzleImageAlt(locale, item, 'card')}</figcaption></figure></Link><div className="puzzle-body"><p className="puzzle-category">{categoryName}</p><h3>{item.name}</h3><div className="puzzle-meta"><span>{localizeAge(tc, item.age)}</span><span>{tc('dots', { count: item.dots })}</span></div><Link href={`/${locale}/${categorySlug}/${item.slug}/`} className="download-link"><Star size={15} aria-hidden="true" /> {tc('viewDownload')}</Link></div></article>)}</div></section>}
      {crossTier.length > 0 && <section className="section" style={{ paddingTop: 8 }}><div className="section-heading"><h2 style={{ fontSize: 'clamp(1.5rem, 3vw, 2.2rem)' }}>{crossTierHeading}</h2></div><div className="puzzle-grid">{crossTier.map((item) => <article className="puzzle-card" key={`${item.categorySlug}-${item.slug}`}><Link href={`/${locale}/${item.categorySlug}/${item.slug}/`} className="puzzle-image"><figure style={{ margin: 0 }}><ResponsiveImage src={item.image} alt={puzzleImageAlt(locale, item, 'card')} width={420} height={320} sizes="(max-width: 760px) 92vw, 30vw" /><figcaption className="sr-only">{puzzleImageAlt(locale, item, 'card')}</figcaption></figure></Link><div className="puzzle-body"><p className="puzzle-category">{item.categoryName}</p><h3>{item.name}</h3><div className="puzzle-meta"><span>{localizeAge(tc, item.age)}</span><span>{tc('dots', { count: item.dots })}</span></div><Link href={`/${locale}/${item.categorySlug}/${item.slug}/`} className="download-link"><Star size={15} aria-hidden="true" /> {tc('viewDownload')}</Link></div></article>)}</div></section>}
