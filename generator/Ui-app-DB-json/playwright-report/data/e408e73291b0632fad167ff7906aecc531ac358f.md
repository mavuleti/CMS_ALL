...
- Name: mobile-and-links.spec.ts >> Home page — content >> benefits section is present
- Location: tests\mobile-and-links.spec.ts:55:7
...
Locator: getByRole('heading', { name: /why dot to dot worksheets are great/i })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('heading', { name: /why dot to dot worksheets are great/i })
...
```ts
  ...
  2   | 
  3   | /* ── Viewports ────────────────────────────────────────────────────────────── */
  4   | const mobileViewports = [
  5   |   { width: 320, height: 720,  name: 'small phone' },
  6   |   { width: 390, height: 844,  name: 'modern phone' },
  7   |   { width: 768, height: 1024, name: 'tablet portrait' }
  8   | ];
  9   | 
  10  | const en = (path = '/') => `/en${path === '/' ? '/' : path}`;
  11  | 
  12  | async function jsonLdByType(page: import('@playwright/test').Page, type: string) {
  13  |   const scripts = await page.locator('script[type="application/ld+json"]').allTextContents();
  14  |   return scripts.find((content) => {
  15  |     try {
  16  |       const data = JSON.parse(content);
  17  |       const types = Array.isArray(data['@type']) ? data['@type'] : [data['@type']];
  18  |       return types.includes(type);
  19  |     } catch {
  20  |       return content.includes(`"@type":"${type}"`);
  21  |     }
  22  |   });
  23  | }
  24  | 
